...
export default function LoginPage() {
  ...
  return <main className="min-h-screen bg-white p-5 sm:grid sm:place-items-center">
    <div className="grid w-full max-w-4xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl sm:grid-cols-2">
      ...
      <section className="p-8 sm:p-10"><h2 className="text-2xl font-bold text-slate-800">Welcome back</h2><p className="mt-2 text-sm text-slate-500">Sign in with your Supabase email and password.</p><form className="mt-7 space-y-5" onSubmit={(event) => void submit(event)}><label className="block text-sm font-medium text-slate-700">Email<input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="employee@company.com" className="mt-1.5 block w-full rounded-md border border-slate-300 px-3 py-2.5 text-slate-800 outline-none focus:border-safety focus:ring-2 focus:ring-orange-100" /></label><div className="block"><label className="block text-sm font-medium text-slate-700">Password</label><div className="relative mt-1.5"><input type={showPassword ? 'text' : 'password'} required value={password} onChange={(event) => setPassword(event.target.value)} placeholder="••••••••" className="block w-full rounded-md border border-slate-300 px-3 py-2.5 pr-10 text-slate-800 outline-none focus:border-safety focus:ring-2 focus:ring-orange-100" /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-600 focus:outline-none">{showPassword ? '👁️‍🗨️' : '👁️'}</button></div></div>{!loading && authUser && profile && !profile.active && <p role="alert" className="rounded-md border border-orange-200 bg-orange-50 p-3 text-sm text-orange-800">Your account is waiting for Admin activation. Ask an administrator to grant module access.</p>}{(error || configurationError) && <p role="alert" className="rounded-md border border-orange-200 bg-orange-50 p-3 text-sm text-orange-800">{error || configurationError}</p>}<button type="submit" disabled={submitting || Boolean(configurationError)} className="w-full rounded-md bg-safety px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-safety-dark disabled:cursor-not-allowed disabled:opacity-60">{submitting ? 'Signing in…' : 'Sign in'}</button></form><div className="mt-5 flex items-center justify-between text-sm"><p className="text-slate-500">New staff member? <Link to="/signup" className="font-semibold text-safety hover:text-safety-dark">Create an account</Link></p></div></section>
    </div>
  </main>;
}
